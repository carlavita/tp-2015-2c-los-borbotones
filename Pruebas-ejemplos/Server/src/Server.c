...
// llena con ceros, por consistencia
	memset(&Msj_Header, '\0', sizeof(t_mensaje_header));

	while (status != 0){
		//recibe la estructura completa
		status = recv(socketCliente,&Msj_Header,sizeof(t_mensaje_header), 0);
		if (status != 0) {
		switch ( Msj_Header.idmensaje) {
					case 2:

						//Envia ack
							puts("Enviar ACK comando");
							sendACK(socketCliente);
						//Recibe estructura
							t_mensaje_estructura Msj;
							recv(socketCliente, &Msj, sizeof(Msj), 0);
