...
int iniciar(int idProceso, int cantidadPaginas) {
	...
	case -2: /* NO HAY ESPACIO PARA ASIGNAR PAGINAS DEVOLVER ERROR */
		log_info(logSWAP,
				"Proceso mProc rechazado por falta de espacido, su PID es %d",
				idProceso);
		return ERROR;
		break;
	default:
		...
		log_info(logSWAP, "Byte Inicial %d",
				paginaInicial * configuracionSWAP.TamanioPagina);
		log_info(logSWAP, "Tamaño en bytes de asignacion %d",
				cantidadPaginas * configuracionSWAP.TamanioPagina);
		return OK;
		break;
	}
	return 0;
...
int finalizar(int PID) {
	...
	log_info(logSWAP, "Byte Inicial %d",
			primerPagina * configuracionSWAP.TamanioPagina);
...
char * leer(int PID, int nroPagina) {
	...
	log_info(logSWAP, "Tamaño solicitado para lectura %d",
...
int escribir(int PID, int nroPagina, char* contenidoPagina) {
	...
	log_info(logSWAP, "PID solicitado: %d", PID);
	log_info(logSWAP, "Byte Inicial %d", primerBytePagina);
	log_info(logSWAP, "Tamaño solicitado para escritura %d",
	...
	while (list_size(listaPaginasLibres) > 1) {
		...
		while (buscarEnPid) {
			...
			if (pos > -1) {
				...
				int primerBytePID = procesoObtenido->primerPagina
						* configuracionSWAP.TamanioPagina;
				int cantidadPaginas = procesoObtenido->ultimaPagina
						- procesoObtenido->primerPagina;
				...
				fseek(archivoDisco,
						DesdeBusqueda * configuracionSWAP.TamanioPagina,
						SEEK_SET);
				fputs(contenido, archivoDisco);
				procesoObtenido->primerPagina = procesoObtenido->primerPagina
						- BloquesLibres - 1;
				procesoObtenido->ultimaPagina = procesoObtenido->ultimaPagina
						- BloquesLibres - 1;
				free(contenido);
				free(contenidoNuevo);
...
void leerArchivoConfiguracion() {
	...
	configuracionSWAP.PuertoEscucha = config_get_int_value(cfgSWAP,
			"PUERTO_ESCUCHA");
	configuracionSWAP.NombreSwap = config_get_string_value(cfgSWAP,
			"NOMBRE_SWAP");
	configuracionSWAP.CantidadPaginas = config_get_int_value(cfgSWAP,
			"CANTIDAD_PAGINAS");
	configuracionSWAP.TamanioPagina = config_get_int_value(cfgSWAP,
			"TAMANIO_PAGINA");
	configuracionSWAP.RetardoSWAP = config_get_int_value(cfgSWAP,
			"RETARDO_SWAP");
	configuracionSWAP.RetardoCompactacion = config_get_int_value(cfgSWAP,
	...
	if (cantidadPaginas > paginasLibres) {
		...
		if (list_size(listaPaginasLibres) == 1) {
			...
			while (contador < list_size(listaPaginasLibres)) {
				...
				if ((pagLibres->hastaPagina - pagLibres->desdePagina)
						>= cantidadPaginas) {
					estadoDevuelto = pagLibres->desdePagina;
...
void * ordenarLista() {
	bool _ordenamiento_porPaginasLibres(t_tablaPaginasLibres* paginasLibres,
			t_tablaPaginasLibres* paginasLibresMenor) {
...
void ordenarListaPorPagina() {
	bool _ordenamiento_porPaginaInicial(t_tablaProcesos* procesoMayor,
			t_tablaProcesos* procesoMenor) {
...
void * unificacionEspacioContiguo() {
	...
	int contador = 0;
	while ((contador + 1 < list_size(listaPaginasLibres))
			&& (list_size(listaPaginasLibres) > 1)) {
...
int buscarBloqueAMover(int DesdePosicion, int HastaPosicion) {
	...
	while (contador < list_size(listaProcesos)) {
		...
		if ((procesoObtenido->primerPagina > DesdePosicion)
				&& (procesoObtenido->ultimaPagina < HastaPosicion)) {
			posicion = contador;
